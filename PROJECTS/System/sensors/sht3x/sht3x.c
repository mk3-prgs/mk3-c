...
#define debug(s, f, ...) xprintf("%s %s: " s "\n", "SHT3x", f, ## __VA_ARGS__)
#define debug_dev(s, f, d, ...) xprintf("%s %s: bus %d, addr %02x - " s "\n", "SHT3x", f, d->bus, d->addr, ## __VA_ARGS__)
#else
#define debug(s, f, ...)
...
#define error(s, f, ...) xprintf("%s %s: " s "\n", "SHT3x", f, ## __VA_ARGS__)
#define error_dev(s, f, d, ...) xprintf("%s %s: bus %d, addr %02x - " s "\n", "SHT3x", f, d->bus, d->addr, ## __VA_ARGS__)
#else
#define error(s, f, ...)
...
bool sht3x_send_command  (sht3x_sensor_t*, uint16_t);
...
sht3x_sensor_t* dev;
    ...
    if((dev = pvPortMalloc(sizeof(sht3x_sensor_t))) == NULL) return NULL;
...
bool sht3x_get_results(sht3x_sensor_t* dev, float* temperature, float* humidity)
{
    if(!dev || (!temperature && !humidity)) return false;
    sht3x_raw_data_t raw_data;
    if(!sht3x_get_raw_data (dev, raw_data)) return false;
    return sht3x_compute_values(raw_data, temperature, humidity);
}
...
static bool sht3x_is_measuring(sht3x_sensor_t* dev)
{
    ...
    if (!dev->meas_started || !dev->meas_first) return false;
    // not running if time elapsed is greater than duration
    uint32_t elapsed = xTaskGetTickCount() - dev->meas_start_time;
    return elapsed < SHT3x_MEAS_DURATION_TICKS[dev->repeatability];
}

int i2c_slave_write(uint8_t bus, uint8_t slave_addr, uint8_t *data, uint32_t len)
{
I2C_TypeDef* i2c;
//
    if(bus > 0) i2c = I2C_2;
...
*/
    ret = I2C_Master_BufferWrite(i2c, data, len, Polling, slave_addr);
...
int i2c_slave_read(uint8_t bus, uint8_t slave_addr, uint8_t *data, uint32_t len)
{
I2C_TypeDef* i2c;
    if(bus > 0) i2c = I2C_2;
    else        i2c = I2C_1;
...
/*
    ...
    i2c_cmd_link_delete(cmd);
*/
    ret = I2C_Master_BufferRead(i2c, data, len, Polling, slave_addr);
    return ret;
}

bool sht3x_send_command(sht3x_sensor_t* dev, uint16_t cmd)
{
    if(!dev) return false;
    ...
    int err = i2c_slave_write(dev->bus, dev->addr, data, 2);
...
{
    if (!dev) return false;
    int err = i2c_slave_read(dev->bus, dev->addr, data, len);
...
#   ifdef SHT3x_DEBUG_LEVEL_2
    xprintf("SHT3x %s: bus %d, addr %02x - read following bytes: ",
...
bool sht3x_reset (sht3x_sensor_t* dev)
{
    if(!dev) return false;
