...
#include "FreeRTOS.h"
//#include "task.h"				/* for malloc, free */
#include "coder.h"
...
MP3DecInfo *AllocateBuffers(void)
{
	...
	len = sizeof(MP3DecInfo);      xprintf("MP3DecInfo: %d\n", len); l += len;
	len = sizeof(FrameHeader);     xprintf("fh:  %d\n", len); l += len;
	len = sizeof(SideInfo);        xprintf("si:  %d\n", len); l += len;
	len = sizeof(ScaleFactorInfo); xprintf("sfi: %d\n", len); l += len;
	len = sizeof(HuffmanInfo);     xprintf("hi:  %d\n", len); l += len;
	len = sizeof(DequantInfo);     xprintf("di:  %d\n", len); l += len;
	len = sizeof(IMDCTInfo);       xprintf("mi:  %d\n", len); l += len;
	len = sizeof(SubbandInfo);     xprintf("sbi: %d\n", len); l += len;
	xprintf("Total %d\n", l);

	mp3DecInfo = (MP3DecInfo *)pvPortMalloc(sizeof(MP3DecInfo));
	if (!mp3DecInfo) return 0;
	...
	xprintf("mp3DecInfo:%08x sbi:%08x mi:%08x fh:%08x si:%08x sfi:%08x hi:%08x di:%08x\n", mp3DecInfo, sbi, mi, fh, si, sfi, hi, di);
	xprintf("Heap: %d\n", xPortGetFreeHeapSize());
...
#define SAFE_FREE(x)	{if (x)	vPortFree(x);	(x) = 0;}	/* helper macro */
