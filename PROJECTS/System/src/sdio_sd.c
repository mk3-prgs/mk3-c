...
void SD_LowLevel_Init(void)
{
  ...
  RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOC | RCC_APB2Periph_GPIOD, ENABLE); //  | SD_DETECT_GPIO_CLK
...
SD_Error SD_Init(void)
{
  ...
  //xprintf("SDIO Peripheral Low Level Init\n");
  SD_LowLevel_Init();
  ...
  //xprintf("Configure the SDIO peripheral Ok!\n");

  if (errorstatus == SD_OK)
  {
    /*----------------- Read CSD/CID MSD registers ------------------*/
    //xprintf("Read CSD/CID MSD registers\n");
...
///
    //SD_CID *c = &SDCardInfo.SD_cid;
    //xprintf("ID=%d SN=%x\n", c->ManufacturerID, c->ProdSN);
///

  if (errorstatus == SD_OK)
  {
    //xprintf("SD_EnableWideBusOperation\n");
    errorstatus = SD_EnableWideBusOperation(SDIO_BusWide_4b);
  }

    //xprintf("SD Init complete\n");
    return(errorstatus);
}
    ...
    if (SDIO_MULTIMEDIA_CARD == CardType) {
        ...
        if (SDIO_BusWide_8b == WideMode) {
            ...
            if(SD_OK == errorstatus) {
                ...
                SDIO_InitStructure.SDIO_HardwareFlowControl = SDIO_HardwareFlowControl_Disable;
                SDIO_Init(&SDIO_InitStructure);
                //xprintf("Configure the SDIO peripheral\n");
                }
            }
        else {
