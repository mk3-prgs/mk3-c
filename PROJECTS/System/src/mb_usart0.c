...
{
    gpio_bit_set(GPIOB, BIT(2));
    return(1);
}

static bool usart_de_clr_callback(void)
{
    gpio_bit_reset(GPIOB, BIT(2));
    return(0);
}
...
/**
 * Инициализирует USART2.
 */
void init_usart0(modbus_rtu_t* p_mb, usart_bus_t* p_ub, int speed)
{
    //
    p_mbus = p_mb;
    p_ubus = p_ub;
    ...
    //
    // USART0: speed b/s, 8 бит данных, контроль чётности - "чётный", 1 стоп бит.
    /*
    ...
    */
    ...
    usart_enable(USART0);
    //xprintf("USART0->CR1=%x\r\n", USART0->CR1);
    //xprintf("USART2->CR1=%x\r\n", USART2->CR1);
    //
    // Шина USART.
    usart_bus_init_t usartb_is = {
        .usart_device = (USART_TypeDef*)USART0,                 // Периферия.
        .dma_tx_channel = p_DMA1_Channel3, // (dma.h) Канал DMA на передачу.
        .dma_rx_channel = p_DMA1_Channel4  // (dma.h) Канал DMA на приём.
        };
    //
...
//
    // Разрешаем прерывания USART0.
    NVIC_SetPriority(USART0_IRQn, 9);
    NVIC_EnableIRQ(USART0_IRQn);

    // Разрешаем прерывания DMA USART0.
    NVIC_SetPriority(DMA0_Channel3_IRQn, 9);
    NVIC_EnableIRQ(DMA0_Channel3_IRQn);

    NVIC_SetPriority(DMA0_Channel4_IRQn, 9);
