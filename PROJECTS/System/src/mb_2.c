...
int data_to_uhr(void);
extern uint16_t uhr[256]; // 0x100 - Регистры user

void init_usart2(modbus_rtu_t* p_mb, usart_bus_t* p_ub, int speed);
...
static modbus_rtu_error_t modbus_on_read_hold_reg(uint16_t mb_addr, uint16_t* value)
{
    ...
    else if((mb_addr >= 0x1000)&&(mb_addr <= 0x1022)) {
        /// доступ к регистрам user
        int adr = (mb_addr - 0x1000);
        int idx = 0x01 & adr;
        adr >>= 1;
        uint16_t* pt16=(uint16_t*)d_tab[adr].pt;
        pt16 += idx;
        *value = *pt16;
        //xprintf("r: a=%04x d=%04x\n", mb_addr, *value);
        }
    else if((mb_addr >= 0x2000)&&(mb_addr <= 0x2040)) {
        //if(mb_addr == 0x2000) data_to_uhr();
        *value = uhr[mb_addr - 0x2000];
        }
    else {
...
static modbus_rtu_error_t modbus_on_write_hold_reg(uint16_t mb_addr, uint16_t value)
{
    ...
    else if((mb_addr >= 0x1000)&&(mb_addr <= 0x1100)) {
...
void mb2_init(uint8_t slave, int speed)
{
    // Инициализация.
    //
    init_usart2(&modbus, &usart_bus, speed);
    init_modbus(slave);
    //
...
//
    ...
    //
    //for(i=0; i<5; i++) {
        ret = xQueueReceive(mb_queue, &stat, 1000);
