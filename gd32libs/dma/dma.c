...
{
    if((uint32_t)dma_channel >= p_DMA1_Channel1_BASE &&
       (uint32_t)dma_channel <= p_DMA1_Channel7_BASE){

        return ((uint32_t)dma_channel - p_DMA1_Channel1_BASE) / 0x14 + 1;

    }else if((uint32_t)dma_channel >= p_DMA2_Channel1_BASE &&
             (uint32_t)dma_channel <= p_DMA2_Channel5_BASE){

        return ((uint32_t)dma_channel - p_DMA2_Channel1_BASE) / 0x14 + 8;
    }
    return 0;
}
